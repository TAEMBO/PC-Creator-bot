module.exports = {
	run: async (client, message, args) => {
		...
		const bestUsers = allUsers.map(x => [x, starboardValues.filter(y => y.a === x && y.c).reduce((a, b) => a.c + b.c, 0)]).filter(x => typeof x[1] === 'number' && !isNaN(x[1])).sort((a, b) => b[1] - a[1]).slice(0, 5).map(x => `<@${x[0]}> | **${x[1]}** :star:`);
