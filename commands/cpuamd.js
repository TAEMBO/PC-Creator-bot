function cpuEmbed(cpu, client) {
	const embed = new client.embed()
		.setTitle(client.cpulist_AMD.manufacturer.charAt(0).toUpperCase() + client.cpulist_AMD.manufacturer.slice(1).toLowerCase() + ' ' + cpu.name)
...
module.exports = {
	run: async (client, message, args) => {
		...
		if (oneResult) {
			const cpu = client.cpulist_AMD[matches.filter(x => x).sort((a, b) => b - a).firstKey()];
			...
			bestMatches.forEach((x, i) => {
				const cpuName = `\`${i + 1}: ${client.cpulist_AMD[x].name}\`\n`;
