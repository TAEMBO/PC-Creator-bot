module.exports = {
    run: async (client, message, args) => {
		...
		const guildMembers = (await message.guild.members.fetch()).filter(x => !x.roles.cache.has(client.config.mainServer.roles.developer));
		let desc = '';
		staff.forEach((role, key) => {
			const members = guildMembers.filter(x => x.roles.cache.has(role.id));
